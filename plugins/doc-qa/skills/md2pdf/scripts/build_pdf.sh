...
for t in pandoc mmdc weasyprint python3; do
  command -v "$t" >/dev/null || { echo "missing: $t" >&2; exit 1; }
done

# CJK text silently turns into empty boxes when no font in the stack below is
# installed. Catch it before rendering. fc-list ships with fontconfig, which
# weasyprint depends on anyway; if it is absent we cannot check, so stay quiet.
if command -v fc-list >/dev/null && [ -z "$(fc-list :lang=zh family 2>/dev/null | head -1)" ]; then
  echo "WARNING: no Chinese font found by fontconfig — CJK text will print as boxes." >&2
  echo "  Debian/Ubuntu/WSL: sudo apt install fonts-noto-cjk" >&2
fi
...
/* One entry per platform, first match wins: macOS, Linux/WSL (fonts-noto-cjk),
   Windows. Without the non-macOS names, weasyprint falls back to a sans-serif
   that usually has no CJK glyphs and every Chinese character becomes a box. */
body { font-family: "PingFang TC","Heiti TC","Noto Sans CJK TC","Noto Sans TC","Source Han Sans TC","Microsoft JhengHei","Arial Unicode MS",sans-serif; font-size: $BODY_PT; line-height: 1.65; color: #222; }
...
code { font-family: "Menlo","Consolas","DejaVu Sans Mono","PingFang TC","Heiti TC","Noto Sans CJK TC","Microsoft JhengHei",monospace; font-size: 9pt; background: #eef1f5; padding: 1px 4px; border-radius: 3px; }
