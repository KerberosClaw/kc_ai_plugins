...
```bash
# Check all four
which pandoc && which mmdc && which weasyprint && which python3
```

Missing tool install commands:

| Tool | macOS | Debian / Ubuntu / WSL |
|---|---|---|
| pandoc | `brew install pandoc` | `sudo apt install pandoc` |
| weasyprint | `brew install weasyprint` | `sudo apt install weasyprint` (or `pip install weasyprint`) |
| mmdc | `npm install -g @mermaid-js/mermaid-cli` | same; headless Chromium may also need `npx puppeteer browsers install chrome-headless-shell` |
| Chinese font | built in | `sudo apt install fonts-noto-cjk` |

**Windows:** run inside WSL. `build_pdf.sh` is a bash script and weasyprint on native Windows needs a separate GTK install; WSL avoids both.

**Script paths:** `scripts/…` below is relative to this skill's own directory (the folder containing this `SKILL.md`), not to the user's project. Call the scripts by that absolute path.
